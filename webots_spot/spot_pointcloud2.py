...
tfs = {
    'left_head_depth': transformation_matrix([0.463, 0.045, 0.030], [-0.032, 0.226, -0.180, 0.957]),
    'right_head_depth': transformation_matrix([0.463, -0.045, 0.028], [0.053, 0.210, 0.180, 0.960]),
    'left_flank_depth': transformation_matrix([-0.136, 0.110, 0.066], [-0.142, 0.142, 0.693, 0.693]),
    'right_flank_depth': transformation_matrix([-0.136, -0.110, 0.066], [0.145, 0.145, -0.692, 0.692]),
    'rear_depth': transformation_matrix([-0.278-0.15, -1.398+1.4, 0.629-0.6], [-0.235, 0.000, 0.972, -0.000]),
...
rectifying_tf= transformation_matrix([0,0,0], [r.as_quat()[0], r.as_quat()[1], r.as_quat()[2], r.as_quat()[3]])
...
class DepthToPCD2(Node):
    def __init__(self) -> None:
        ...
        self.create_subscription(Image,"/Spot/right_head_depth", self.frontright_fn, 10)
    ...
    def frontleft_fn(self,msg:Image) -> None:
        self.frontleft_img = self.bridge.imgmsg_to_cv2(msg, desired_encoding='passthrough')
        self.check_fl = True

    def frontright_fn(self,msg:Image) -> None:
        self.frontright_img = self.bridge.imgmsg_to_cv2(msg, desired_encoding='passthrough')
        self.check_fr = True
    ...
    def run(self):
        if not self.count_subscribers('/Spot/pointcloud2'):
            return
        if self.check_fl and self.check_fr and self.check_l and self.check_r and self.check_b:
            self.check_fl = False
            self.check_fr = False
            self.check_l = False
            self.check_r = False
